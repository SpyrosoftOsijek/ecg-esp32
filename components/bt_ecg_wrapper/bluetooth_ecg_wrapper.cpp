...
static uint8_t advertizing_service_uuid[16] = {0xfb, 0x34, 0x9a, 0x5f, 0x80, 0x00, 0x00, 0x80,
...
void start_service_and_add_char(std::uint16_t service_handle)
{
    ...
    ESP_LOGW(BLE_ECG_TAG, "Service started, characteristic added");
}
...
bool BluetoothECGWrapper::isConnected()
{
    return connected;
}
...
void BluetoothECGWrapper::esp_gatts_cb(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    {   
        ESP_LOGW(BLE_ECG_TAG, "ESP_GATTS_REG_EVT");  
        if (!connected)
        {
    ...
    case ESP_GATTS_CREATE_EVT:
    {
        ...
        ESP_LOGW(BLE_ECG_TAG, "ESP_GATTS_CREATE_EVT triggered");
    ...
    {
        ESP_LOGW(BLE_ECG_TAG, "ESP_GATTS_ADD_CHAR_EVT");
    ...
    {
        ESP_LOGW(BLE_ECG_TAG, "ESP_GATTS_CONNECT_EVT");
    ...
    {
        ESP_LOGW(BLE_ECG_TAG, "ESP_GATTS_DISCONNECT_EVT triggered %x",param->disconnect.reason);
